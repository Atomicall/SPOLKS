...
class Server:
    ...
    def work(self):
        while True:
            try:
                readable, writable, exceptional = select(self._inputs, self._outputs, self._inputs, 1.0)
    ...
    def _init_sockets(self):
        ...
        print(f'TCP Server created on address: {ip.get_local_ip_address(is_local_host=IS_LOCAL_HOST)}:{self.TCP_PORT}')
        self._set_udp_socket()
        print(f'UDP Server created on address: {ip.get_local_ip_address(is_local_host=IS_LOCAL_HOST)}:{self.UDP_PORT}')
