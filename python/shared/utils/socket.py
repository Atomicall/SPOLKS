...
def _set_keepalive_osx(sock: socket.socket,
                       keep_alive_interval: int = 3):
    """
    Set TCP keepalive on an open socket.
    ...
    """
    # scraped from /usr/include, not exported by python's socket
    # module
    TCP_KEEPALIVE = 0x10
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
    sock.setsockopt(
...
def _set_keepalive_windows(sock: socket.socket, keep_alive_time: int = 1,
                           keep_alive_interval: int = 3, max_probes: int = 5):
    """
    Set TCP keepalive on an open socket.
...
def set_socket_keep_alive(
    ...
    if system == 'Windows':
        _set_keepalive_windows(
            sock, keep_alive_time, keep_alive_interval)
    elif system == 'Linux':
        _set_keepalive_linux(
            sock,
            keep_alive_time,
            keep_alive_interval,
            max_probes)
