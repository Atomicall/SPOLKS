...
from shared.udp.packet import Acknowledgment, Packet
from shared.utils.console import progress


def send(sock: socket.socket, init_address: Tuple[str, int],
         packets: List[Packet], window_size: int = 0, show_progress: bool = False):
    if window_size == 0:
        window_size = len(packets) if len(
            packets) < WINDOWS_SIZE else WINDOWS_SIZE
    ...
    while sequence_number < length:
        try:
            ...
            if ready[0]:
                ...
                # timed out
                unacknowledged_number = 0
            if unacknowledged_number < window_size and (
                    unacknowledged_number + sequence_number) < length:
                sock.sendto(pickle.dumps(
                    packets[unacknowledged_number + sequence_number]), init_address)
                unacknowledged_number += 1
                continue
        except socket.error:
            continue
...
def receive(sock: socket.socket, initial_address: Tuple[str, int] = (
        '', ''), file_name: str = ''):
    expected_sequence = 0
    prev_sequence = 0
    timeout = 0.2
        ...
        while True:
            try:
                ...
                if prev_sequence == data.sequence_number and is_finished:
                    # skip first ack in case sender just didn't catch
                    # up with the ack
                    if stub == 0:
                        stub += 1
                        continue
